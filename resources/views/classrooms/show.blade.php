<x-main-layout title="View">
    <x-navbar :area="route('classrooms.show' , $classroom->id )" :ass="route('classrooms.classworks.index' , $classroom->id)" :people="route('classrooms.people' , $classroom->id)"/>
          <div class="container">
                ...
                <p style="top: 410px;left:150px;" class="position-absolute text-white"><a class="link-light" href="{{ $invitation_link }}">{{ $invitation_link }}</a></p>
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <div class="d-flex justify-content-between mb-5">
                            ...
                            <a href="{{route('classrooms.classworks.index' , $classroom->id)}}" class="btn btn-success">{{__("Classworks")}}</a>
                        </div>
                    </div>
                    <div class="col-md-9">
                        <form action="{{ route('posts.store' , $classroom->id)}}" method="POST">
                            ...
                            <div class="input-group mb-4 shadow bg-white post-collapse" id="post-area" style="border: none">
                                ...
                                <input type="text" name="content" class="form-control p-4 post-input" placeholder="{{__("Write a post...")}}" aria-label="announcement" aria-describedby="basic-addon1">
                        ...
                        <li class="list-group-item border rounded p-3 mb-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <p class="mb-0">{{ $post->content }} {{__("By")}} {{ $post->user->name }}</p>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form action="{{ route('comments.store') }}" method="post">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn bg-green text-white" data-bs-dismiss="modal">Close</button>
                                                <button type="submit" class="btn bg-green text-white">Submit Comment</button>
                                            </div>
                                        </form>
                                    </div>
                        ...
                        <li class="list-group-item border rounded p-3 mb-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-center">
                                    <img class="rounded-circle me-3" src="https://ui-avatars.com/api/?name={{ $stream->user->name }}" width="40" height="40" alt="{{ $stream->user->name }}">
                                    <div>
                                        <p class="mb-0">{{ $stream->content }} {{__("By")}} {{ $stream->user->name }}</p>
                                        <p class="mb-0 small text-muted">{{ $stream->created_at->diffForHumans() }}</p>
                                    </div>
                                </div>
                                {{-- <div class="dropdown">
                                    <button class="border-0 bg-white dropdown-toggle" type="button" id="postMenu{{ $stream->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="fas fa-ellipsis-v"></i>
                                    </button>
                                    <ul class="dropdown-menu" aria-labelledby="postMenu{{ $stream->id }}">
                                        <li><a class="dropdown-item" href="#">{{__("Edit")}}</a></li>
                                        <li>
                                            <form action="{{ route('posts.destroy', $stream->id) }}" method="stream">
                                                @csrf
                                                @method('delete')
                                                <button type="submit" class="dropdown-item">{{__("Delete")}}</button>
                                            </form>   
